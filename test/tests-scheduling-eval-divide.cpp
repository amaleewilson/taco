...
int div_WARP_SIZE = 32;

void div_printToCout(IndexStmt stmt) {
...
IndexStmt div_scheduleSpMVCPU(IndexStmt stmt, int CHUNK_SIZE=16) {
  ...
  return stmt.divide(i, i0, i1, CHUNK_SIZE)
          .reorder({i0, i1, j})
          .parallelize(i0, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces);
}

IndexStmt div_scheduleSpMMCPU(IndexStmt stmt, Tensor<double> A, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar i0("i0"), i1("i1"), kbounded("kbounded"), k0("k0"), k1("k1"), jpos("jpos"), jpos0("jpos0"), jpos1("jpos1");
  return stmt.divide(i, i0, i1, CHUNK_SIZE)
          .pos(j, jpos, A(i,j))
          .divide(jpos, jpos0, jpos1, UNROLL_FACTOR)
          .reorder({i0, i1, jpos0, k, jpos1})
          .parallelize(i0, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces)
          .parallelize(k, ParallelUnit::CPUVector, OutputRaceStrategy::IgnoreRaces);
}

IndexStmt div_scheduleSDDMMCPU(IndexStmt stmt, Tensor<double> B, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar i0("i0"), i1("i1"), kpos("kpos"), kpos0("kpos0"), kpos1("kpos1");
  return stmt.divide(i, i0, i1, CHUNK_SIZE)
          .pos(k, kpos, B(i,k))
          .divide(kpos, kpos0, kpos1, UNROLL_FACTOR)
          .reorder({i0, i1, kpos0, j, kpos1})
          .parallelize(i0, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces)
          .parallelize(kpos1, ParallelUnit::CPUVector, OutputRaceStrategy::ParallelReduction);
}

IndexStmt div_scheduleTTVCPU(IndexStmt stmt, Tensor<double> B, int CHUNK_SIZE=16) {
  IndexVar f("f"), fpos("fpos"), chunk("chunk"), fpos2("fpos2");
  return stmt.fuse(i, j, f)
          .pos(f, fpos, B(i,j,k))
          .divide(fpos, chunk, fpos2, CHUNK_SIZE)
          .reorder({chunk, fpos2, k})
          .parallelize(chunk, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces);
}

IndexStmt div_scheduleTTMCPU(IndexStmt stmt, Tensor<double> B, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar f("f"), fpos("fpos"), chunk("chunk"), fpos2("fpos2"), kpos("kpos"), kpos1("kpos1"), kpos2("kpos2");
  return stmt.fuse(i, j, f)
          .pos(f, fpos, B(i,j,k))
          .divide(fpos, chunk, fpos2, CHUNK_SIZE)
          .pos(k, kpos, B(i,j,k))
          .divide(kpos, kpos1, kpos2, UNROLL_FACTOR)
          .reorder({chunk, fpos2, kpos1, l, kpos2})
          .parallelize(chunk, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces)
          .parallelize(kpos2, ParallelUnit::CPUVector, OutputRaceStrategy::ParallelReduction);;
}

IndexStmt div_scheduleMTTKRPCPU(IndexStmt stmt, Tensor<double> B, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar i1("i1"), i2("i2");
  return stmt.divide(i, i1, i2, CHUNK_SIZE)
          .reorder({i1, i2, k, l, j})
          .parallelize(i1, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces);
}

IndexStmt div_scheduleMTTKRPPrecomputedCPU(IndexStmt stmt, Tensor<double> B, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar i1("i1"), i2("i2"), j_pre("j_pre");
  return stmt.divide(i, i1, i2, CHUNK_SIZE)
          .parallelize(i1, ParallelUnit::CPUThread, OutputRaceStrategy::NoRaces);
}

IndexStmt div_scheduleMTTKRP4CPU(IndexStmt stmt, Tensor<double> B, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar i1("i1"), i2("i2");
  return stmt.divide(i, i1, i2, CHUNK_SIZE)
...
IndexStmt div_scheduleMTTKRP5CPU(IndexStmt stmt, Tensor<double> B, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar i1("i1"), i2("i2");
  return stmt.divide(i, i1, i2, CHUNK_SIZE)
...
IndexStmt div_scheduleSpMVGPU(IndexStmt stmt, Tensor<double> A, IndexExpr precomputedExpr, int NNZ_PER_THREAD=8, int BLOCK_SIZE=256) {
  int NNZ_PER_WARP = NNZ_PER_THREAD * div_WARP_SIZE;
  ...
  return stmt.fuse(i, j, f)
          .pos(f, fpos, A(i, j))
          .divide(fpos, block, fpos1, NNZ_PER_TB)
          .divide(fpos1, warp, fpos2, NNZ_PER_WARP)
          .divide(fpos2, thread, thread_nz, NNZ_PER_THREAD)
...
IndexStmt div_scheduleSpMVRowsGPU(IndexStmt stmt, Tensor<double> A, IndexExpr precomputedExpr, int ROWS_PER_WARP=1, int BLOCK_SIZE=256) {
  ...
  TensorVar precomputed("precomputed", Type(Float64, {Dimension(thread_nz)}), taco::dense);
  return stmt.divide(i, block, block_row, ROWS_PER_TB)
          .divide(block_row, warp_row, warp, BLOCK_SIZE / div_WARP_SIZE)
          .pos(j, jpos, A(i, j))
          .divide(jpos, thread_nz, thread, div_WARP_SIZE)
...
IndexStmt div_scheduleSpMVThreadPerRowGPU(IndexStmt stmt, Tensor<double> A, IndexExpr precomputedExpr, int BLOCK_SIZE=256) {
  int ROWS_PER_TB = BLOCK_SIZE;
  IndexVar block("block"), warp("warp"), thread("thread"), thread_nz("thread_nz"), i1("i1"), jpos("jpos"), block_row("block_row"), warp_row("warp_row");
  return stmt.divide(i, block, thread, ROWS_PER_TB)
...
IndexStmt div_scheduleSpMVDividePosGPU(IndexStmt stmt, Tensor<double> A, IndexExpr precomputedExpr, int NNZ_PER_THREAD=8, int BLOCK_SIZE=256) {
  int NNZ_PER_WARP = NNZ_PER_THREAD * div_WARP_SIZE;
  int NNZ_PER_TB = NNZ_PER_THREAD * BLOCK_SIZE;
  IndexVar f("f"), fpos("fpos"), fpos1("fpos1"), fpos2("fpos2"), block("block"), warp("warp"), thread("thread"), thread_nz("thread_nz"), thread_nz_pre("thread_nz_pre");
  TensorVar precomputed("precomputed", Type(Float64, {Dimension(thread_nz)}), taco::dense);
...
IndexStmt div_scheduleSpMMGPU(IndexStmt stmt, Tensor<double> A, IndexExpr precomputedExpr, int NNZ_PER_WARP=8, int BLOCK_SIZE=256) {
  int NNZ_PER_TB = NNZ_PER_WARP * (BLOCK_SIZE / div_WARP_SIZE);
  ...
  return stmt.reorder({i, j, k})
          ...
          .pos(f, fpos, A(i, j))
          .divide(fpos, block, fpos1, NNZ_PER_TB)
          .divide(fpos1, warp, nnz, NNZ_PER_WARP)
          .divide(k, dense_val_unbounded, thread, div_WARP_SIZE)
...
IndexStmt div_scheduleSDDMMGPU(IndexStmt stmt, Tensor<double> B, int NNZ_PER_WARP=8*32, int BLOCK_SIZE=256, int CO_FACTOR=4) {
  int NNZ_PER_TB = NNZ_PER_WARP * (BLOCK_SIZE / div_WARP_SIZE);
  ...
  return stmt.reorder({i, k, j})
          ...
          .divide(fpos, block, fpos1, NNZ_PER_TB)
          .divide(fpos1, warp, nnz, NNZ_PER_WARP)
          .divide(j, dense_val_unbounded, thread, div_WARP_SIZE)
...
IndexStmt div_scheduleTTMGPU(IndexStmt stmt, Tensor<double> B, int NNZ_PER_WARP=8*32, int BLOCK_SIZE=256, int CO_FACTOR=4) {
  int NNZ_PER_TB = NNZ_PER_WARP * (BLOCK_SIZE / div_WARP_SIZE);
  ...
  return stmt.reorder({i, j, k, l})
          ...
          .divide(fpos, block, fpos1, NNZ_PER_TB)
          .divide(fpos1, warp, nnz, NNZ_PER_WARP)
          .divide(l, dense_val_unbounded, thread, div_WARP_SIZE)
          .bound(dense_val_unbounded, dense_val, CO_FACTOR, BoundType::MaxExact)
          .reorder({block, warp, nnz, thread, dense_val})
          .unroll(dense_val, CO_FACTOR)
          .parallelize(block, ParallelUnit::GPUBlock, OutputRaceStrategy::IgnoreRaces)
...
IndexStmt div_scheduleTTVGPU(IndexStmt stmt, Tensor<double> B, IndexExpr precomputedExpr, int NNZ_PER_WARP=8*32, int BLOCK_SIZE=256) {
  int NNZ_PER_TB = NNZ_PER_WARP * (BLOCK_SIZE / div_WARP_SIZE);
  ...
  return stmt.fuse(j, k, jk)
          ...
          .divide(fpos, block, fpos1, NNZ_PER_TB)
          .divide(fpos1, warp, fpos2, NNZ_PER_WARP)
          .divide(fpos2, thread, thread_nz, NNZ_PER_WARP/div_WARP_SIZE)
          .reorder({block, warp, thread, thread_nz})
          .precompute(precomputedExpr, thread_nz, thread_nz_pre, precomputed)
          .unroll(thread_nz_pre, NNZ_PER_WARP/div_WARP_SIZE)
          .parallelize(block, ParallelUnit::GPUBlock, OutputRaceStrategy::IgnoreRaces)
          .parallelize(warp, ParallelUnit::GPUWarp, OutputRaceStrategy::IgnoreRaces)
          .parallelize(thread, ParallelUnit::GPUThread, OutputRaceStrategy::Atomics);
}

IndexStmt div_scheduleMTTKRPGPU(IndexStmt stmt, Tensor<double> B, int NNZ_PER_WARP=16, int BLOCK_SIZE=256) {
  int NNZ_PER_TB = NNZ_PER_WARP * (BLOCK_SIZE / div_WARP_SIZE);
...
// divides so same number of rows per warp and then each thread in warp gets 1/32 of the columns space
IndexStmt div_scheduleSpMMRowsGPU(IndexStmt stmt, Tensor<double> A, int ROWS_PER_WARP=4, int BLOCK_SIZE=256) {
  int ROWS_PER_TB = ROWS_PER_WARP * (BLOCK_SIZE / div_WARP_SIZE);
  IndexVar i1("i1"), block("block"), warp("warp"), warp_row("warp_row"), thread("thread"), thread_col("thread_col");
  return stmt.divide(i, block, i1, ROWS_PER_TB)
          .divide(i1, warp, warp_row, ROWS_PER_WARP)
          .divide(k, thread, thread_col, 32)
...
// divides so same number of nonzero rows per warp and then each thread in warp gets 1/32 of the columns space (no search needed)
IndexStmt div_scheduleSpMMNZRowsGPU(IndexStmt stmt, Tensor<double> A, int NZ_ROWS_PER_WARP=4, int BLOCK_SIZE=256) {
  int NZ_ROWS_PER_TB = NZ_ROWS_PER_WARP * (BLOCK_SIZE / div_WARP_SIZE);
  ...
  return stmt.pos(i, ip, A(i, j))
          .divide(ip, block, ip1, NZ_ROWS_PER_TB)
          .divide(ip1, warp, warp_row, NZ_ROWS_PER_WARP)
          .divide(k, thread, thread_col, 32)
          .reorder({block, warp, warp_row, thread, thread_col, j})
          .parallelize(block, ParallelUnit::GPUBlock, OutputRaceStrategy::IgnoreRaces)
          .parallelize(warp, ParallelUnit::GPUWarp, OutputRaceStrategy::IgnoreRaces)
...
IndexStmt div_scheduleSpMMCPUNoVec(IndexStmt stmt, Tensor<double> A, int CHUNK_SIZE=16, int UNROLL_FACTOR=8) {
  IndexVar i0("i0"), i1("i1"), kbounded("kbounded"), k0("k0"), k1("k1"), jpos("jpos"), jpos0("jpos0"), jpos1("jpos1");
  return stmt.divide(i, i0, i1, CHUNK_SIZE)
          .pos(j, jpos, A(i,j))
          .divide(jpos, jpos0, jpos1, UNROLL_FACTOR)
...
IndexStmt div_exampleScheduleSPMVUntiled(IndexStmt stmt, Tensor<double> A) {
  return stmt;
}

IndexStmt div_exampleScheduleSPMVCPURowTiling(IndexStmt stmt, Tensor<double> A) {
  IndexVar i1("i1"), i2("i2");
  int ROWS_PER_TILE = 4;
  return stmt.divide(i, i1, i2, ROWS_PER_TILE);
}

IndexStmt div_exampleScheduleSPMVPosIteration(IndexStmt stmt, Tensor<double> A) {
  IndexVar f("f"), p("p");
  return stmt.fuse(i, j, f)
             .pos(f, p, A(i, j));
}
...
TEST(div_scheduling_eval, div_test_spmvCPU_temp) {
  ...
  //div_printToFile("test_spmvCPU_temp", stmt);
...
TEST(div_scheduling_eval, example_spmvCPU_dividepos) {
  ...
  stmt = stmt.fuse(i, j, k)
          .pos(k, kpos, A(i, j))
          .divide(kpos, kpos0, kpos1, CHUNK_SIZE)
          .parallelize(kpos0, ParallelUnit::CPUThread, OutputRaceStrategy::Atomics);

  //div_printToFile("example_spmv_cpu_dividepos", stmt);
...
TEST(div_scheduling_eval, div_spmmCPU) {
  ...
  stmt = div_scheduleSpMMCPU(stmt, A);

  //div_printToFile("spmm_cpu", stmt);
...
TEST(div_scheduling_eval, div_sddmmCPU) {
  ...
  stmt = div_scheduleSDDMMCPU(stmt, B);

  //div_printToFile("sddmm_cpu", stmt);
...
TEST(div_scheduling_eval, div_spmvCPU) {
  ...
  IndexStmt stmt = y.getAssignment().concretize();
  stmt = div_scheduleSpMVCPU(stmt);

  //div_printToFile("spmv_cpu", stmt);
...
TEST(div_scheduling_eval, div_ttvCPU) {
  ...
  IndexStmt stmt = A.getAssignment().concretize();
  stmt = div_scheduleTTVCPU(stmt, B);

  //div_printToFile("ttv_cpu", stmt);
...
TEST(div_scheduling_eval, div_ttmCPU) {
  ...
  IndexStmt stmt = A.getAssignment().concretize();
  stmt = div_scheduleTTMCPU(stmt, B);

  //div_printToFile("ttm_cpu", stmt);
...
TEST(div_scheduling_eval, div_mttkrpCPU) {
  ...
  IndexStmt stmt = A.getAssignment().concretize();
  stmt = div_scheduleMTTKRPCPU(stmt, B);
  //div_printToFile("mttkrp_cpu", stmt);
...
TEST(div_scheduling_eval, div_spmvGPU) {
  ...
  IndexStmt stmt = y.getAssignment().concretize();
  stmt = div_scheduleSpMVGPU(stmt, A, precomputed);

  //div_printToFile("spmv_gpu", stmt);
...
TEST(div_scheduling_eval, div_spmmGPU) {
  ...
  IndexStmt stmt = C.getAssignment().concretize();
  stmt = div_scheduleSpMMGPU(stmt, A, precomputed);

  //div_printToFile("spmm_gpu", stmt);
...
TEST(div_scheduling_eval, div_spmmDCSRGPU) {
  ...
  IndexStmt stmt = C.getAssignment().concretize();
  stmt = div_scheduleSpMMNZRowsGPU(stmt, A);

  //div_printToFile("spmm_dcsr_gpu", stmt);
...
TEST(div_scheduling_eval, div_sddmmGPU) {
  ...
  IndexStmt stmt = A.getAssignment().concretize();
  stmt = div_scheduleSDDMMGPU(stmt, B);

  //div_printToFile("sddmm_gpu", stmt);
...
TEST(div_scheduling_eval, div_ttmGPU) {
  ...
  IndexStmt stmt = A.getAssignment().concretize();
  stmt = div_scheduleTTMGPU(stmt, B);

  //div_printToFile("ttm_gpu", stmt);
...
TEST(div_scheduling_eval, div_ttvGPU) {
  ...
  IndexStmt stmt = A.getAssignment().concretize();
  stmt = div_scheduleTTVGPU(stmt, B, precomputedExpr);

  //div_printToFile("ttv_gpu", stmt);
...
TEST(div_scheduling_eval, div_mttkrpGPU) {
  ...
  IndexStmt stmt = A.getAssignment().concretize();
  stmt = div_scheduleMTTKRPGPU(stmt, B);

  //div_printToFile("mttkrp_gpu", stmt);
...
TEST(div_generate_evaluation_files, DISABLED_cpu) {
  if (should_use_CUDA_codegen()) {
    return;
  }
  ...
  // spmv
  {
    ...
    for (auto paramSet : spmv_parameters) {
      IndexStmt div_scheduled = div_scheduleSpMVCPU(stmt, paramSet[0]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
  ...
  // spmm
  {
    ...
    for (auto paramSet : spmm_parameters) {
      IndexStmt div_scheduled = div_scheduleSpMMCPU(stmt, A, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // sddmm
  {
    ...
    for (auto paramSet : sddmm_parameters) {
      IndexStmt div_scheduled = div_scheduleSDDMMCPU(stmt, B, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // ttv
  {
    ...
    for (auto paramSet : ttv_parameters) {
      IndexStmt div_scheduled = div_scheduleTTVCPU(stmt, B, paramSet[0]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // ttm
  {
    ...
    for (auto paramSet : ttm_parameters) {
      IndexStmt div_scheduled = div_scheduleTTMCPU(stmt, B, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // mttkrp3
  {
    ...
    for (auto paramSet : mttkrp_parameters) {
      IndexStmt div_scheduled = div_scheduleMTTKRPCPU(stmt, B, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // mttkrp3 workspace
  {
    ...
    IndexStmt div_scheduled = div_scheduleMTTKRPPrecomputedCPU(precomputed_stmt, B, 64);
  ...
  // mttkrp4
  {
    ...
    bool isFirst = true;
    for (auto paramSet : mttkrp_parameters) {
      IndexStmt div_scheduled = div_scheduleMTTKRP4CPU(stmt, B, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // mttkrp5
  {
    ...
    bool isFirst = true;
    for (auto paramSet : mttkrp_parameters) {
      IndexStmt div_scheduled = div_scheduleMTTKRP5CPU(stmt, B, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
...
TEST(div_generate_evaluation_files, DISABLED_gpu) {
  if (!should_use_CUDA_codegen()) {
    return;
  }
  ...
  // spmv load-balance
  {
    ...
    IndexStmt div_scheduled = div_scheduleSpMVRowsGPU(stmt, A, precomputed);
    ir::Stmt compute = lower(div_scheduled, string("compute_warp_row"),  false, true);
    codegen->compile(compute, isFirst);
    isFirst = false;

    div_scheduled = div_scheduleSpMVThreadPerRowGPU(stmt, A, precomputed);
    compute = lower(div_scheduled, string("compute_thread_row"),  false, true);
    codegen->compile(compute, isFirst);
  ...
  // spmv
  {
    ...
    bool isFirst = true;
    for (auto paramSet : spmv_parameters) {
      IndexStmt div_scheduled = div_scheduleSpMVGPU(stmt, A, precomputed, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // spmm
  {
    ...
    for (auto paramSet : spmm_parameters) {
      ...
      IndexStmt stmt = C.getAssignment().concretize();
      IndexStmt div_scheduled = div_scheduleSpMMGPU(stmt, A, precomputed, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // sddmm
  {
    ...
    bool isFirst = true;
    for (auto paramSet : sddmm_parameters) {
      int NUM_K = paramSet[2] * div_WARP_SIZE;
      std::shared_ptr<ir::CodeGen> codegen = ir::CodeGen::init_default(source, ir::CodeGen::ImplementationGen);
      Tensor<double> A("A", {NUM_I, NUM_K}, {Dense, Dense});
      Tensor<double> B("B", {NUM_I, NUM_K}, CSR);
      Tensor<double> D("D", {NUM_J, NUM_K}, {Dense, Dense});
      A(i,k) = B(i,k) * C(i,j) * D(j,k);
      IndexStmt stmt = A.getAssignment().concretize();
      IndexStmt div_scheduled = div_scheduleSDDMMGPU(stmt, B, paramSet[0], paramSet[1], paramSet[2]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // ttv
  {
    ...
    bool isFirst = true;
    for (auto paramSet : ttv_parameters) {
      IndexStmt div_scheduled = div_scheduleTTVGPU(stmt, B, precomputedExpr, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // ttm
  {
    ...
    for (auto paramSet : ttm_parameters) {
      ...
      A(i,j,l) = B(i,j,k) * C(k,l);
      IndexStmt stmt = A.getAssignment().concretize();
      IndexStmt div_scheduled = div_scheduleTTMGPU(stmt, B, paramSet[0], paramSet[1], paramSet[2]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
  ...
  // mttkrp
  {
    ...
    bool isFirst = true;
    for (auto paramSet : mttkrp_parameters) {
      int NUM_J = div_WARP_SIZE;
      Tensor<double> A("A", {NUM_I, NUM_J}, {Dense, Dense});
      Tensor<double> C("C", {NUM_K, NUM_J}, {Dense, Dense});
      Tensor<double> D("D", {NUM_L, NUM_J}, {Dense, Dense});
      A(i,j) = B(i,k,l) * C(k,j) * D(l,j);
      IndexStmt stmt = A.getAssignment().concretize();
      IndexStmt div_scheduled = div_scheduleMTTKRPGPU(stmt, B, paramSet[0], paramSet[1]);
      ir::Stmt compute = lower(div_scheduled, string("compute_") + util::join(paramSet, "_"),  false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
    }
...
TEST(div_generate_figures, DISABLED_cpu) {
  if (should_use_CUDA_codegen()) {
    return;
  }
  ...
  // spmv
  {
    ...
    IndexStmt  (* schedulingFunctions [])(IndexStmt, Tensor<double>) = {&div_exampleScheduleSPMVUntiled, &div_exampleScheduleSPMVCPURowTiling, &div_exampleScheduleSPMVPosIteration};

    int ii = 0;
    for (auto schedulingFunction : schedulingFunctions) {
      IndexStmt div_scheduled = schedulingFunction(stmt, A);
      ir::Stmt compute = lower(div_scheduled, functionNames[ii], false, true);
      codegen->compile(compute, isFirst);
      isFirst = false;
      ii++;
    }
