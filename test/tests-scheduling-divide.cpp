...
TEST(div_scheduling, forallReplace) {
  IndexVar i1, j1, j2;
...
TEST(div_scheduling, divideIndexStmt) {
  Type t(type<double>(), {3});
  TensorVar a("a", t), b("b", t);
  IndexVar i1, i2;
  IndexStmt stmt = forall(i, a(i) = b(i));
  IndexStmt divideStmt = stmt.divide(i, i1, i2, 2);
...
TEST(div_scheduling, lowerDenseMatrixMul) {
...
TEST(div_scheduling, lowerSparseCopy) {
...
TEST(div_scheduling, lowerSparseMulDense) {
  Tensor<double> A("A", {8}, {Sparse});
...
TEST(div_scheduling, lowerSparseMulSparse) {
  Tensor<double> A("A", {8}, {Sparse});
...
TEST(div_scheduling, lowerSparseAddSparse) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Sparse});
  Tensor<double> C("C", {8}, {Dense});
...
TEST(div_scheduling, lowerSparseMatrixMul) {
...
TEST(div_scheduling, parallelizeAtomicReduction) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Dense});
  Tensor<double> C("C");
...
TEST(div_scheduling, parallelizeTemporaryReduction) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Dense});
  Tensor<double> C("C");
...
TEST(div_scheduling, multilevel_tiling) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Sparse});
  Tensor<double> C("C", {8}, {Dense});
...
TEST(div_scheduling, pos_mul_dense) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Dense});
  Tensor<double> C("C", {8}, {Dense});
...
TEST(div_scheduling, pos_mul_sparse) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Sparse});
  Tensor<double> C("C", {8}, {Dense});
...
TEST(div_scheduling, pos_mul_dense_divide) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Dense});
  Tensor<double> C("C", {8}, {Dense});
  ...
  IndexStmt stmt = C.getAssignment().concretize();
  stmt = stmt.pos(i, ipos, A(i)).divide(ipos, iposOuter, iposInner, 2);
...
TEST(div_scheduling, pos_tile_coord_and_pos) {
  Tensor<double> A("A", {8}, {Sparse});
  Tensor<double> B("B", {8}, {Dense});
  Tensor<double> C("C", {8}, {Dense});
  ...
  IndexStmt stmt = C.getAssignment().concretize();
  stmt = stmt.divide(i, iOuter, iInner, 4)
          .pos(iInner, ipos, A(i)).divide(ipos, iposOuter, iposInner, 2);
...
TEST(div_scheduling, spmv_warp_per_row) {
  if (!should_use_CUDA_codegen()) {
    return;
  }
  ...
  stmt = stmt.divide(i, block, block_row, ROWS_PER_BLOCK)
          .divide(block_row, warp_row, warp, WARPS_PER_BLOCK)
          .pos(j, jpos, A(i, j))
          .divide(jpos, thread_element, thread, WARP_SIZE)
...
TEST(div_scheduling, dense_pos_error) {
...
TEST(div_scheduling, pos_var_not_in_access) {
  Tensor<double> x("x", {8}, {Dense});
  Tensor<double> y("y", {8}, {Dense});
...
TEST(div_scheduling, pos_wrong_access) {
  Tensor<double> x("x", {8}, {Dense});
  Tensor<double> y("y", {8}, {Dense});
  IndexVar i("i"), ipos("ipos"), j("j");
  y(i) = x(i);
...
TEST(div_scheduling_eval_test, spmv_fuse) {
